...
describe('任务详情：渲染', () => {
	it('渲染标题、编号与基础字段', async () => {
		...
		// done=false + percent_done=0.5 → 状态“进行中”被高亮为当前态
		expect(within(screen.getByTestId('detail-done')).getByText('进行中')).toHaveAttribute(
			'data-active',
			'true',
		);
...
describe('任务详情：乐观更新与错误', () => {
	it('勾选后立刻反映在界面上', async () => {
		...
		await waitFor(() =>
			expect(within(screen.getByTestId('detail-done')).getByText('已完成')).toHaveAttribute(
				'data-active',
				'true',
			),
		);
	...
	it('★ 点状态“已完成”发 done=true，并回传 assignees（不清空指派）', async () => {
		const mock = mockTask(task({ assignees: [{ id: 99, username: 'zoe' }] }));
		renderApp(`/tasks/${TASK_ID}`);
		await screen.findByRole('heading', { name: '写文档' });

		await userEvent.click(within(screen.getByTestId('detail-done')).getByText('已完成'));

		await waitFor(() => expect(mock.posts).toHaveLength(1));
		expect(mock.posts[0]!.done).toBe(true);
		// ⚠️ 全量替换不带 assignees 会清空指派 —— 必须原样回传
		expect(mock.posts[0]!.assignees).toEqual([{ id: 99, username: 'zoe', name: '' }]);
	});

	it('★ 点“待办”发 done=false + percent_done=0', async () => {
		const mock = mockTask(task({ done: true, percent_done: 1 }));
		renderApp(`/tasks/${TASK_ID}`);
		await screen.findByRole('heading', { name: '写文档' });

		await userEvent.click(within(screen.getByTestId('detail-done')).getByText('待办'));

		await waitFor(() => expect(mock.posts).toHaveLength(1));
		expect(mock.posts[0]!.done).toBe(false);
		expect(mock.posts[0]!.percent_done).toBe(0);
	});
